...
def mobilenet_CAM(img_path, model, all_amp_layer_weights):
    ...
    mat_for_mult = scipy.ndimage.zoom(last_conv_output, (32, 32, 1), order = 1)  # dim from (16, 16, 1024) to (256, 256, 1024)
    ...
    final_output = np.dot(mat_for_mult, amp_layer_weights) # dim: 256 x 256
...
def plot_CAM(img_path, ax, model, all_amp_layer_weights):
    # Loading the image / resizing to 256x256 / Converting BGR to RGB
    #im = cv2.resize(cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB), (256, 256))
...
def prediction_localisation(img_path):
    ensemble_model = starter()
    ...
    plt.savefig(directory+"/static/result_images/image1.png")
    # Second image
    plt.subplot(122)
    ...
    plt.savefig(directory+"/static/result_images/image2.png")
    img = cv2.imread(directory+"/static/result_images/image2.png")
    ...
    cv2.imwrite(directory+"/static/result_images/image2.png", resized)
    plt.show()
    path = directory+"\\static\\result_images\\image2.png"
    return path